      ...
      <div style={{ 
        ...
        {result && (
          <div style={{ 
            ...
            <div style={{
              ...
              <div style={{ 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'flex-end' }}>
                  <div style={{
                    ...
                    🧠 {result.security_analysis.analysis_method}
                  </div>
                  <div style={{
                    ...
                    🤖 {result.security_analysis.model_type}
                  </div>
                  <div style={{
                    background: result.summary.status === 'VULNERABLE' 
                      ? 'linear-gradient(135deg, #fee2e2, #fca5a5)' 
                      : 'linear-gradient(135deg, #dcfce7, #86efac)',
                    padding: '6px 12px',
                    borderRadius: '16px',
                    fontSize: '11px',
                    color: result.summary.status === 'VULNERABLE' ? '#dc2626' : '#16a34a',
                    border: `1px solid ${result.summary.status === 'VULNERABLE' ? '#fca5a5' : '#86efac'}`,
                    fontWeight: '600'
                  }}>
                    {result.summary.status === 'VULNERABLE' ? '🚨' : '✅'} {result.summary.status}
                  </div>
                </div>
              </div>
              ...
              <div style={{ 
                ...
                <div style={{ 
                  ...
                  <span>🎯</span>
                  Logic Consistent: {result.security_analysis.trained_model_result.raw_result.logic_consistent ? '✅' : '❌'}
                </div>
              </div>
            </div>
            ...
            {/* Risk Summary Card */}
            {(() => {
              const severity = getSeverityLevel(result.security_analysis.risk_level);
                  ...
                  <div style={{ 
                    ...
                    <div>
                      ...
                      <p style={{ margin: '4px 0 0 0', opacity: 0.9 }}>
                        {result.security_analysis.trained_model_result.recommendation}
                      </p>
                    </div>
                  </div>
                  ...
                  <div style={{ 
                    ...
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '2.5rem', fontWeight: '800', marginBottom: '4px' }}>
                        {Math.round(result.security_analysis.overall_vulnerability_score * 100)}%
                      </div>
                      <div style={{ fontSize: '13px', opacity: 0.9 }}>Risk Skoru</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '2.5rem', fontWeight: '800', marginBottom: '4px' }}>
                        {result.summary.total_vulnerabilities_found}
                      </div>
                      <div style={{ fontSize: '13px', opacity: 0.9 }}>Zafiyet Sayısı</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '2.5rem', fontWeight: '800', marginBottom: '4px' }}>
                        {Math.round(result.security_analysis.trained_model_result.confidence * 100)}%
                      </div>
                      <div style={{ fontSize: '13px', opacity: 0.9 }}>Güvenilirlik</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '1.8rem', fontWeight: '800', marginBottom: '4px' }}>
                        {result.security_analysis.trained_model_result.max_severity}
                      </div>
                      <div style={{ fontSize: '13px', opacity: 0.9 }}>En Yüksek Tehdit</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '2.5rem', fontWeight: '800', marginBottom: '4px' }}>
                        {result.security_analysis.trained_model_result.raw_result.base_score.toFixed(1)}
                      </div>
                      <div style={{ fontSize: '13px', opacity: 0.9 }}>Base Score</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '2rem', fontWeight: '800', marginBottom: '4px' }}>
                        {result.security_analysis.trained_model_result.vulnerability_count}
                      </div>
                      <div style={{ fontSize: '13px', opacity: 0.9 }}>Model Count</div>
                    </div>
                  </div>
                </div>
              );
            ...
            {result.security_analysis.trained_model_result.raw_result.primary_vulnerability && (
              <div style={{
                background: 'rgba(255, 255, 255, 0.95)',
                borderRadius: '20px',
                padding: '24px',
                ...
                {(() => {
                  const primaryVuln = result.security_analysis.trained_model_result.raw_result.primary_vulnerability;
                  const severityConfig = {
                      ...
                      <div style={{
                        ...
                        <div style={{ 
                          ...
                          <div style={{ fontSize: '16px', fontWeight: '700', color: config.text }}>
                            {primaryVuln.is_real_vulnerability ? '✅ Confirmed' : '❌ False Positive'}
                          </div>
                        </div>
            ...
            {result.security_analysis.trained_model_result.vulnerabilities.length > 0 && (
              <div style={{
                background: 'rgba(255, 255, 255, 0.95)',
                borderRadius: '20px',
                padding: '24px',
                ...
                {result.security_analysis.trained_model_result.vulnerabilities.map((vuln, index) => {
                  const severityConfig = {
                    'CRITICAL': { bg: '#fef2f2', border: '#dc2626', text: '#dc2626' },
                    'HIGH': { bg: '#fff7ed', border: '#ea580c', text: '#ea580c' },
                    'MEDIUM': { bg: '#fffbeb', border: '#d97706', text: '#d97706' },
                  ...
                  return (
                    <div key={index} style={{
                      ...
                      borderRadius: '16px',
                      padding: '20px',
                      marginBottom: index < result.security_analysis.trained_model_result.vulnerabilities.length - 1 ? '16px' : '0'
                    }}>
                      <div style={{ 
                        display: 'flex', 
                        alignItems: 'center', 
            ...
            <div style={{
              ...
              <div style={{ display: 'grid', gap: '12px' }}>
                {Object.entries(result.security_analysis.cwe_analysis).map(([cwe, data]) => (
                  <div key={cwe} style={{
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'space-between',
